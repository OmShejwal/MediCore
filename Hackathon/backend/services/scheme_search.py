...
class SchemeSearchEngine:
    def __init__(self):
        ...
        # Keep Sahayak functional even when optional search deps are unavailable.
        self.web_search = None
        if DuckDuckGoSearchRun is not None:
            try:
                self.web_search = DuckDuckGoSearchRun()
            except Exception as e:
                print(f"DuckDuckGo search disabled: {e}")

    def search_schemes(self, user_profile: Dict, question: Optional[str] = None) -> Dict:
        if not self.llm:
            return {
                "answer": "Real-time scheme search is currently in demo mode. Please configure GROQ_API_KEY for AI-ranked live scheme responses. Meanwhile, common options include ADIP, UDID benefits, and state disability pension schemes.",
                "schemes": [],
                "sources": [],
                "query": (question or "")
            }
        ...
        try:
            # 2. Fetch real-time results from DuckDuckGo (if available)
            ...
            if self.web_search is not None and (not web_results or len(web_results) < 50):
            ...
            if not web_results:
                web_results = "No live web search results available. Use user profile and general Indian disability scheme knowledge."
